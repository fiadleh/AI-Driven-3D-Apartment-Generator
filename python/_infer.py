...
def run_model(graph_data, floorplan_samples):
	print('run_model', flush=True)
	#return
	#return
	# parse json
	...
	# model.half()Modern living room with stylish decor
	print("load model: --- %s seconds ---" % (time.time() - start_time), flush=True)
	...
	for k in range(floorplan_samples):
		start_time = time.time()
		print(f'=== Generating Floorplan {k} ====')
		...
		# send masks
		start_time = time.time()
		im_svg = draw_masks(masks.copy(), real_nodes, im_size=256, floorplan_id=k)
		print("check time: --- %s seconds ---" % (round(time.time() - start_time,2)))
		yield '<stop>' + str(im_svg)
		#print('<stop>' + str(im_svg))
	print("Floorplanes Generation done !!!!!!\n\n")