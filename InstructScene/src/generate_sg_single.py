...
def main():
    ...
    save_dir = os.path.join(exp_dir, "generated_scenes", str(hashlib.md5(args.room_prompt.encode('utf-8')).hexdigest()))
    ...
    for epoch in range(args.n_epochs):
        ...
        for batch_idx, batch in tqdm(
            ...
            #print(texts)
            #exit()
            texts = ['Put a white dressing table to the right of a metal wardrobe.', 
                     'position a grey nightstand to the left of a wooden wardrobe with doors. Hang a white cieling lamp with a wooden circle directly above a black double bed.', 
                     'Position a black and white circular pendant lamp behind a black and silver nightstand with a drawer.', 
                     'Place a wooden wardrobe without doors to the right of a red nightstand with a drawer.', 
                     "A cozy bedroom with a large window and a white bed and a yellow nightstand."]
            
            texts = ["A cozy bedroom with a large window and a white bed and a yellow nightstand."]
            texts = [args.room_prompt]
            ...
            for i in range(len(bbox_params_t)):
                ...
                print(tr_floor)
                """
                # Create a trimesh scene anexport_scene(d export it to a temporary directory
                ii = epoch * len(dataset) + batch_idx * B + i
                export_dir = save_dir #os.path.join(save_dir, f"{ii:04d}@{batch['scene_uids'][i]}_cfg{args.cfg_scale:.1f}_{args.sg2sc_cfg_scale:.1f}")
                tmp_dir = os.path.join(export_dir, "tmp")
                os.makedirs(export_dir, exist_ok=True)
                os.makedirs(tmp_dir, exist_ok=True)
                with open(os.path.join(export_dir, "info.json"), "w") as outfile:
                    json.dump([x_min, z_min, x_max, z_max], outfile)
                print('export all objects to a temp folder: ', tmp_dir)
                #obj_classes = ['' if v is None else v for v in obj_classes]
                #print(obj_classes)
                export_scene(tmp_dir, trimesh_meshes, bbox_meshes, obj_classes)

                print(' --------- Rendering Room only Topview  -----------')
                ...
                print('## Visualize the generated scene graph ##')
