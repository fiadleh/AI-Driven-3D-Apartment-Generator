...
class ThreedFutureDataset(object):
    ...
    def get_closest_furniture_to_objfeat_and_size(self, query_label, query_size, query_objfeat, objfeat_type):
        ...
        for oi in objects:
            feature_attr = getattr(oi, f"{objfeat_type}_features", None)
            if feature_attr is None:
                print(f"Skipping object {oi} due to missing {objfeat_type}_features")
                exit()
                continue
            
            if not np.allclose(np.linalg.norm(feature_attr, axis=-1), 1.0):
                print(f"Object {oi} has non-normalized features.")
                exit()
                continue
            #print(feature_attr, '\n***********************************************\n', query_objfeat,'\n###########################################\n')
            #exit()
            cos_sims[oi] = np.dot(feature_attr, query_objfeat)
        
        if not cos_sims:
            raise ValueError("No valid objects with features found for the specified query.")
    ...
    def get_closest_furniture_to_objfeat_and_size2(self, query_label, query_size, query_objfeat, objfeat_type):
        ...
        objects = self._filter_objects_by_label(query_label)
        print(objects, query_label, query_size, query_objfeat, objfeat_type,'\n-------------------\n')
